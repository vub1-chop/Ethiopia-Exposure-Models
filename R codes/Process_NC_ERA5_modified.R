...
a <- read.csv("/Users/lenovo/Desktop/HaSET/ERA5_process/Codes/ThiessenFishnet_1km_noborder_to250mGrid_Pro.csv",as.is=T)
...
era <- read.csv("/Users/lenovo/Desktop/HaSET/ERA5_process/Codes/ERA5_StudyRegion_Pro.csv",as.is=T)

# Directory of satellite data
Dir <-paste("/Users/lenovo/Desktop/HaSET/ERA5_land/2020/",sep="")
# Get file names
...
for (fi in 1:length(file1)){
  ...
  # seq calculates the total number of pairings between lon and lat
  ...
  year <- rep(substr(t[1],1,4), length(t))
  month <- rep(substr(t[1],6,7), each=length(t))
  day <- rep(formatC(1:(length(t)/24), width = 2, flag = "0"), each=24)
  ...
  c = b[vars[v]]
    ...
    for (ti in 1:length(day_begin)){
      ...
      # write.csv(dat2, file = "/Users/lenovo/Desktop/HaSET/ERA5_process/test/ERA5_StudyRegion_d2m_20180101_fulltest.csv", row.names=F)
      ...
      # write.csv(dat4, file = "/Users/lenovo/Desktop/HaSET/ERA5_process/test/ERA5_StudyRegion_d2m_20180101_test2.csv", row.names=F)
  ...
  for (da in 1:length(unique(day))){
    ...
    saveRDS(a5, file = paste0("/Users/lenovo/Desktop/HaSET/ERA5_land/processed/",unique(year),"/Ethiopia_Processed_ERA5_StudyRegion_250m_",dates,".rds"))

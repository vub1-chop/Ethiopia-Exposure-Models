...
# Read in 1km Grid; need "foreign" library to readc dbf from shapefile
...
# read in date
date <- read.csv("D:/HaSET/Codes/date/date2018.csv",colClasses=c("character","character","character"))
...
for (i in 341:length(year)){
  
  # Read in Meteosat grid; need "foreign" library to read dbf from shapefile
  meteosat <- read.csv("D:/HaSET/Codes/Grids/Meteosat_MH_StudyRegion.csv",as.is=T)
  
  # MLE for each day
  Dir <-paste0("D:/HaSET/Raw_NC/Meteosat_MLE/",year[i],"/",month[i],"/",day[i],"/")
  ...
  for (fi in 1:length(file1)){
    ...
    MLE <- merge %>% 
      mutate(!!time[fi] := as.numeric(sub(" \\[W/m^2\\]", "", merge$MLE))) %>% 
      dplyr::select(time[fi]) %>% 
      mutate(Num=seq(1:nrow(merge)))
    
    
    meteosat <- merge(meteosat,MLE,by="Num")
  ...
  MLE_mean <- na.omit(MLE_mean)
  
  # interpolation
  coordinates(MLE_mean) <- ~ X_Pro + Y_Pro
  
  # Interpolation -- "gstat" library needed for idw function, searching 4 nearest neighbors
  krigged <- idw(mean ~ 1, MLE_mean, ExtendedGrid, nmax=4)
  result <- data.frame(krigged$var1.pred)
  colnames(result) <- paste0("MLE")
  a3 <- cbind(a3,result)
  
  saveRDS(a3, file = paste0("D:/HaSET/Processed/Meteosat_MLE/",year[i],"/Ethiopia_Processed_Meteosat_MLE_StudyRegion_250m_",year[i],month[i],day[i],".rds"))

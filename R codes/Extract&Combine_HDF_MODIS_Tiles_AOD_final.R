...
Dir <-paste("D:/HaSET/Raw_HDF/MODIS_AOD/2023/",sep="")
# Dir <-paste("E:/Bryan/Projects/Ethiopia/MODIS/AOD/",sep="")
# Get file names
file1 <- sort(list.files(Dir))                    
#years <- substr(file1,10,13)
#doy <- substr(file1,14,16)
days <- substr(file1,9,16)
filename <- substr(file1,10,16)
dates <- unique(days)
dates_filename <- unique(filename)
...
for (fi in 25:34){
  ...
  write.csv(alldat, file = paste0("D:/HaSET/Processed/MODIS_AOD/2023/MODIS_AOD_RAW_",dates_filename[fi],".csv"), row.names=F)
  print(paste0("Finished date - ",dates_filename[fi]))
}

# write.csv(missingness, file = paste0("D:/HaSET/Processed/MODIS_AOD/2018/MODIS_AOD_RAW_MissingnessTable_2018.csv"), row.names=F)
...
index <- which(num_file==8)
print(index)



master <- read.csv("D:/HaSET/Processed/MODIS_AOD/2023/MODIS_AOD_RAW_2023002.csv")

test <- read.csv("D:/HaSET/Processed/MODIS_AOD/2023/MODIS_AOD_RAW_2023226.csv")


# for dates with >4 files
mean <- test %>%
  group_by(ID) %>% 
  summarise(mean_47=mean(Optical_Depth_047,na.rm=TRUE),
            mean_55=mean(Optical_Depth_055,na.rm=TRUE))

all_dat <- merge(mean,master,by="ID")

all_dat <- all_dat %>% 
  select(-Optical_Depth_047,-Optical_Depth_055) %>% 
  rename(Optical_Depth_047=mean_47,
         Optical_Depth_055=mean_55)

write.csv(all_dat, file="D:/HaSET/Processed/MODIS_AOD/2023/MODIS_AOD_Raw_2023226.csv",row.names = FALSE)

# rework
num_file <- c()
for (fi in 1:length(dates)){
  cal <- length(grep(dates[fi],file1))
  num_file <- c(num_file,cal)
}
print(num_file)

...
Dir <-paste("D:/HaSET/Raw_HDF/MODIS_Terra_Land_Surface_Temperature/2023/",sep="")
...
for (fi in 1:length(dates)){
  ...
  write.csv(alldat, file = paste0("D:/HaSET/Processed/MODIS_Terra_LST/2023/MODIS_Terra_LST_RAW_",dates[fi],".csv"), row.names=F)
  print(paste0("Finished date - ",dates[fi]))
}

write.csv(missingness, file = paste0("D:/HaSET/Processed/MODIS_Terra_LST/missingness_new/MODIS_Terra_LST_RAW_MissingnessTable_2023.csv"), row.names=F)
...
# 
# # master file
# test1 <- read.csv("D:/HaSET/Processed/MODIS_Terra_LST/2023/MODIS_Terra_LST_RAW_A2023001.csv")
# 
# dir <- paste("D:/HaSET/Processed/MODIS_Terra_LST/2023/rework/",sep="")
# lst <- sort(list.files(dir)) 
# 
# for (i in 1:length(lst)){
#   
#   test <- read.csv(paste0(dir,lst[i]))
#   
#   mean <- test %>%
#     group_by(ID) %>%
#     summarise(mean_lst=mean(LST_1KM,na.rm=TRUE))
#   
#   all_dat <- merge(mean,test1,by="ID")
#   
#   all_dat <- all_dat %>%
#     select(-LST_1KM) %>%
#     rename(LST_1KM=mean_lst)
#   
#   write.csv(all_dat, file=paste0("D:/HaSET/Processed/MODIS_Terra_LST/2023/",lst[i]),row.names = FALSE)
# }
# 
# 

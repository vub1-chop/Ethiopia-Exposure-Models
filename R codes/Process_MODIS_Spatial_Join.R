...
dir_aod <-paste("D:/HaSET/Processed/MODIS_AOD/2023/",sep="")
file_aod <- sort(list.files(dir_aod)) 

# directory for processed ERA5
dir_era5 <- paste("D:/HaSET/Processed/EAR5/processed/2023/", sep="")
file_era5 <- sort(list.files(dir_era5))

# directory for population density
dir_population_density <- paste("D:/HaSET/Processed/Population_Density/2023/",sep="")
...
for (i in 1:length(dates_2023)){
  ...
  match_modis_grid$date <- ymd(dates_2023[i])
...
# saveRDS(rm_na,file=paste0("D:/HaSET/Processed/spatial_join_result/no_na_new/MODIS_rm_na_",year[i],".rds"))       

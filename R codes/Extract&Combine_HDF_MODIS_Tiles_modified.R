...
for (fi in 1:length(dates)){
  ...
  write.csv(alldat, file = paste0("D:/HaSET/Processed/MODIS_AOD/2023/MODIS_AOD_RAW_",dates[fi],".csv"), row.names=F)
  print(paste0("Finished date - ",dates[fi]))
}

write.csv(missingness, file = paste0("D:/HaSET/Processed/MODIS_AOD/2023/MODIS_AOD_RAW_MissingnessTable_2023.csv"), row.names=F)
print("Finished - 2023")

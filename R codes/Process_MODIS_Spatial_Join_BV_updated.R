...
master3 <- master2[,c("grid_id","modis_id")]
...
for (y in 2020:2023){
  ...
  era5 <- era5 %>% 
    ...
    # add year, month, day
    gap_filling$year <- year(gap_filling$date)
    gap_filling$month <- month(gap_filling$date)
    gap_filling$day <- day(gap_filling$date)
    
    # daily file for AOD, LST and ERA5
    # write.csv(match_modis_grid, file=paste0("D:/HaSET/Processed/spatial_join_result/updated_0915/",y,"/Spatial_Join_Full_data_250m_",year[i],"_",month[i],"_",day[i],".csv"))
    
    saveRDS(gap_filling, file=paste0("D:/HaSET/Processed/spatial_join_result/updated_0915/",year[i],"/Spatial_Join_Full_data_250m_",year[i],"_",month[i],"_",day[i],".rds"))
